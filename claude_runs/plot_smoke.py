"""
Plot script created by Claude — a copy of ../plot.py that reads the checkpoint
from claude_runs/result/ and writes figures to claude_runs/plot/, so nothing in
the original result/ or plot/ folders is touched.

Run from the repo root:
    conda run -n sleap-new python claude_runs/plot_smoke.py
"""
import os, sys
# allow running from the repo root OR from claude_runs/: put repo root on the path
...
# ── Load saved results (claude_runs, not the originals) ──────────────────────
result_path = './claude_runs/result/sim_model_hidden8_fold0.pt'
...
# ── Run eval ─────────────────────────────────────────────────────────────────
y_pred_np, _, pos_test, *_ = train.eval_(model, rnninfer, X_test, y_test, device)
inferred_states = np.exp(pos_test).argmax(axis=-1)     # (trials, time)
true_states     = label_test[:, :, 0].astype(int)      # (trials, time)

os.makedirs('./claude_runs/plot', exist_ok=True)

# ── Quick numeric summary printed to stdout ──────────────────────────────────
mse_per_trial = np.array([
    mean_squared_error(y_test_np[i].ravel(), y_pred_np[i].ravel())
    for i in range(len(y_test_np))
])
state_acc = np.mean([
    balanced_accuracy_score(true_states[i], inferred_states[i])
    for i in range(len(true_states))
])
# states are permutation-invariant; report the better of label/flipped-label match
state_acc_flip = np.mean([
    balanced_accuracy_score(true_states[i], 1 - inferred_states[i])
    for i in range(len(true_states))
])
print(f"epochs trained         : {len(m1['loss_train'])}")
print(f"mean recon MSE (test)  : {mse_per_trial.mean():.4f}")
print(f"state balanced-acc     : {max(state_acc, state_acc_flip):.3f}  "
      f"(raw {state_acc:.3f} / flipped {state_acc_flip:.3f})")

# ── Plot 1: Neural reconstruction (best test trial) ──────────────────────────
trial  = int(np.argmin(mse_per_trial))
y_true = y_test_np[trial].T
...
ax.set_title('Neural Reconstruction (claude_runs)', fontsize=10)
ax.set_xlabel('Time')
...
cbar.set_ticks([vmin, (vmin + vmax) / 2, vmax]); cbar.set_ticklabels(['Low', 'Mid', 'High'])
plt.tight_layout()
plt.savefig('./claude_runs/plot/neural_recon.png', dpi=150, bbox_inches='tight')
print('Saved: ./claude_runs/plot/neural_recon.png'); plt.close()
...
axes[0].set_title('True States', fontsize=10);     axes[0].set_ylabel('Trial'); axes[0].set_xticks([])
axes[1].imshow(inferred_states, aspect='auto', cmap=cmap_states, vmin=0, vmax=num_tv-1, interpolation='nearest')
axes[1].set_title('Inferred States', fontsize=10); axes[1].set_ylabel('Trial'); axes[1].set_xlabel('Time')
plt.tight_layout()
plt.savefig('./claude_runs/plot/states.png', dpi=150, bbox_inches='tight')
print('Saved: ./claude_runs/plot/states.png'); plt.close()
